...
export default function PortfolioCard({ 
  ...
  const handleVideoTimeUpdate = (e) => {
    ...
    // Limit preview to 10 seconds for non-members / public view
  ...
  const renderMedia = () => {
    if (type === 'image') {
      return fileUrl ? (
        ...
        <div className="aspect-video bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 gap-2 rounded-2xl border border-slate-100 dark:border-slate-800">
    ...
    return (
      <div className="aspect-video bg-blue-50/50 dark:bg-blue-950/20 flex flex-col items-center justify-center text-blue-600 dark:text-blue-400 p-6 rounded-2xl border border-blue-100/40 dark:border-blue-900/30 relative overflow-hidden transition-all duration-300 group-hover:bg-blue-50 dark:group-hover:bg-blue-950/35">
        <FileText size={38} className="mb-2 text-blue-600 dark:text-blue-500 stroke-1 transition-transform duration-300 group-hover:scale-110" />
        ...
        <span className="text-xs text-slate-500 dark:text-slate-500 mt-1">{fileSize || 'Unknown size'}</span>
        ...
        {fileUrl && (
          <a
            ...
            download={fileName}
            onClick={(e) => e.stopPropagation()} 
            className="mt-3 flex items-center gap-1.5 px-4 py-1.5 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 active:scale-95 text-xs font-bold rounded-xl text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 shadow-sm transition duration-150 cursor-pointer"
          >
            <Download size={13} />
            <span>ดาวน์โหลด</span>
          </a>
    ...
    >
      ...
      <div className="flex-1 flex flex-col px-1">
        <div className="flex items-center gap-2 mb-3">
          ...
          >
            <div className="w-5.5 h-5.5 rounded-full overflow-hidden bg-slate-200 dark:bg-slate-900 flex items-center justify-center text-[9px] font-bold text-slate-500 dark:text-slate-400 shrink-0 border border-slate-100 dark:border-slate-800">
              {authorAvatar ? (
            ...
            <div className="flex items-center gap-1">
              ...
              {authorId && <span className="text-[10px] text-slate-400 dark:text-slate-600 font-medium">@{authorId}</span>}
            </div>
          </div>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-100 dark:border-slate-800 mt-auto">
          <div className="flex flex-wrap gap-2 max-w-[50%]">
            {tags && tags.slice(0, 2).map((tag, idx) => (
              <span 
                ...
                className="text-xs text-slate-500 hover:text-blue-600 dark:text-slate-600 dark:hover:text-blue-400 transition-colors cursor-pointer"
              >
                #{tag}
              </span>
            ))}
          ...
          <div className="flex items-center gap-3.5">
            <button
              ...
              className="flex items-center gap-1 text-xs font-bold text-slate-500 dark:text-slate-500 hover:text-rose-500 dark:hover:text-rose-400 transition-colors cursor-pointer group/like"
              title="ถูกใจผลงานนี้"
            >
              <Heart 
                size={14} 
            ...
            {(canEdit || canDelete) ? (
              <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                {canEdit && (
                  <button
                    ...
                    className="text-slate-400 dark:text-slate-600 hover:text-blue-600 dark:hover:text-blue-400 p-1.5 rounded hover:bg-slate-50 dark:hover:bg-slate-800 active:scale-90 transition cursor-pointer"
                    title="แก้ไข"
                  >
                    <Edit3 size={15} />
                  </button>
                ...
                {canDelete && (
                  <button
                    ...
                    className="text-slate-400 dark:text-slate-600 hover:text-rose-500 p-1.5 rounded hover:bg-slate-50 dark:hover:bg-slate-800 active:scale-90 transition cursor-pointer"
                    title="ลบ"
                  >
                    <Trash2 size={15} />
                  </button>
              ...
              type === 'image' && fileUrl && (
                <a 
                  ...
                  rel="noreferrer"
                  onClick={(e) => e.stopPropagation()} 
                  className="text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 flex items-center gap-0.5 text-xs font-bold cursor-pointer transition-colors"
                >
                  <span>เปิดดูรูป</span>
