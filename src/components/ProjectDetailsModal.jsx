...
export default function ProjectDetailsModal({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="bg-white dark:bg-slate-900 rounded-[32px] overflow-hidden border border-slate-100 dark:border-slate-800 max-w-2xl w-full relative z-10 shadow-2xl shadow-slate-200/50 dark:shadow-black/35 opacity-0 scale-95 animate-scale-in text-slate-700 dark:text-slate-350 flex flex-col max-h-[90vh]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800 shrink-0">
          ...
          <button 
            onClick={onClose}
            className="p-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-655 rounded-full transition active:scale-90 cursor-pointer"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="overflow-y-auto p-6 space-y-6">
          ...
          {/* Media Preview */}
          <div className="rounded-2xl overflow-hidden bg-slate-50 dark:bg-slate-950 border border-slate-100 dark:border-slate-850">
            {type === 'image' && fileUrl && (
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 py-3 border-y border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-3">
                ...
                >
                  <div className="w-9 h-9 rounded-full overflow-hidden bg-slate-200 dark:bg-slate-900 flex items-center justify-center text-xs font-bold text-slate-500 border border-slate-100 dark:border-slate-855 shrink-0 transition group-hover/author:border-blue-500">
                    {authorAvatar ? (
                  ...
                  <div>
                    ...
                    <div className="text-[10px] md:text-xs text-slate-400 dark:text-slate-550">{authorEmail}</div>
                  </div>
                </div>
                ...
                {currentUser && currentUser.email !== authorEmail && (
                  <button
                    ...
                    className={`px-3 py-1 rounded-full text-[10px] font-bold transition active:scale-95 cursor-pointer border ${
                      isFollowing
                        ? 'bg-slate-100 dark:bg-slate-850 text-slate-600 dark:text-slate-400 border-slate-200/50 dark:border-slate-800'
              ...
              <div className="flex items-center gap-4 text-xs text-slate-400 dark:text-slate-500">
                ...
                <button
                  ...
                  className="flex items-center gap-1 text-xs font-bold text-slate-455 dark:text-slate-555 hover:text-rose-500 dark:hover:text-rose-400 transition-colors cursor-pointer group/like mr-2"
                  title="ถูกใจผลงานนี้"
                >
                  <Heart 
                    size={14} 
                  ...
                  />
                  <span className={isLiked ? 'text-rose-600 dark:text-rose-450' : 'text-slate-500 dark:text-slate-400'}>{likeCount}</span>
                </button>
            ...
            {/* Author Bio & Social Link inside details modal */}
                ...
                <div className="flex flex-wrap gap-2">
                  {tags.map((tag, idx) => (
                    <span 
                      ...
                      className="text-xs px-2.5 py-1 bg-slate-50 dark:bg-slate-955 border border-slate-100 dark:border-slate-850 hover:border-blue-500/40 text-slate-500 dark:text-slate-400 rounded-lg hover:text-blue-600 cursor-pointer transition duration-150"
                    >
                      #{tag}
                    </span>
                  ))}
            ...
            <div className="border-t border-slate-100 dark:border-slate-800 pt-6 space-y-4">
              <h4 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider flex items-center gap-1.5">
                ...
                <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-950 text-slate-505 dark:text-slate-400 rounded-full text-[10px] font-extrabold">
              ...
              <div className="space-y-4 max-h-[220px] overflow-y-auto pr-1">
                {item.comments && item.comments.length > 0 ? (
                  item.comments.map((comment) => {
                    ...
                    return (
                      <div key={comment.id} className="flex gap-3 text-xs md:text-sm animate-[fadeIn_0.2s_ease-out]">
                        ...
                        <div className="w-8 h-8 rounded-full overflow-hidden bg-slate-100 dark:bg-slate-955 flex items-center justify-center shrink-0 border border-slate-200/50 dark:border-slate-800 font-bold text-slate-400">
                        ...
                        {/* Comment Content */}
                        <div className="flex-1 bg-slate-50 dark:bg-slate-950 border border-slate-100/80 dark:border-slate-850/60 p-3.5 rounded-2xl">
                            ...
                            <div className="flex items-center gap-2 shrink-0">
                              ...
                              {(isOwnComment || isAdmin) && (
                                <button
                                  ...
                                  className="text-[10px] font-bold text-rose-500 hover:text-rose-650 dark:text-rose-400 hover:underline cursor-pointer transition active:scale-95"
                                >
                                  ลบ
                                </button>
                              )}
                  ...
                  <p className="text-xs text-slate-400 dark:text-slate-550 italic py-2">ยังไม่มีความคิดเห็นในขณะนี้</p>
                )}
              </div>
              ...
              {currentUser ? (
                <form 
                  ...
                  className="flex gap-2.5 items-end pt-2"
                >
                  <textarea
                    name="commentText"
                    ...
                    required
                    rows={1}
                    className="flex-1 px-4 py-2.5 text-xs md:text-sm bg-slate-50 dark:bg-slate-950 border border-slate-205 dark:border-slate-800 focus:border-slate-400 dark:focus:border-slate-650 rounded-xl outline-none text-slate-855 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-555 transition-colors resize-none"
                    onKeyDown={(e) => {
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 bg-slate-900 hover:bg-slate-850 dark:bg-slate-100 dark:hover:bg-slate-200 text-white dark:text-slate-900 rounded-xl text-xs font-bold transition cursor-pointer active:scale-95 shadow-xs"
                  >
                    ส่ง
                  </button>
                </form>
        ...
        {type === 'image' && fileUrl && (
          <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 flex justify-end shrink-0">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="px-5 py-2.5 bg-slate-900 dark:bg-slate-100 hover:bg-slate-855 dark:hover:bg-slate-200 text-white dark:text-slate-900 text-xs font-bold rounded-2xl active:scale-[0.98] transition cursor-pointer"
            >
              เปิดภาพขนาดจริง
            </a>
          </div>
