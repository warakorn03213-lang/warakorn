...
export default function FilterModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="bg-white dark:bg-slate-900 rounded-[32px] overflow-hidden border border-slate-100 dark:border-slate-800 max-w-lg w-full relative z-10 shadow-2xl shadow-slate-200/50 dark:shadow-black/35 opacity-0 scale-95 animate-scale-in text-slate-700 dark:text-slate-355 flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800 shrink-0">
          ...
          <button 
            onClick={onClose}
            className="p-1.5 hover:bg-slate-50 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-350 rounded-full transition active:scale-90 cursor-pointer"
          >
            <X size={18} />
          </button>
        </div>
        ...
        <div className="overflow-y-auto p-6 space-y-6">
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-slate-450 dark:text-slate-500 uppercase tracking-wider">เลือกประเภทผลงาน</h4>
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-2.5 p-3.5 rounded-2xl border text-xs font-bold transition duration-150 cursor-pointer active:scale-95 ${
                      isSelected
                        ? 'bg-blue-50 dark:bg-blue-955/20 border-blue-500 text-blue-600 dark:text-blue-400 shadow-xs'
          ...
          {/* Section 2: Tags Selector */}
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-slate-450 dark:text-slate-500 uppercase tracking-wider">เลือกแท็กผลงาน</h4>
            {allTags.length === 0 ? (
              <p className="text-xs text-slate-400 dark:text-slate-550">ไม่มีแท็กในระบบผลงานขณะนี้</p>
            ) : (
                    ...
                    <button
                      ...
                      className={`px-3.5 py-1.5 rounded-full text-xs font-semibold border transition duration-150 cursor-pointer active:scale-95 ${
                        isSelected
                          ...
                          : 'bg-slate-50 dark:bg-slate-955 border-slate-200/50 dark:border-slate-800 text-slate-650 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:border-slate-350 dark:hover:border-slate-700'
                      }`}
                    >
                      #{tag}
                    </button>
        ...
        <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 flex items-center justify-between shrink-0">
          <button
            ...
            className="text-xs font-bold text-slate-450 hover:text-rose-500 dark:text-slate-500 dark:hover:text-rose-455 transition cursor-pointer active:scale-95"
          >
            ล้างตัวกรองทั้งหมด
          </button>
